...
const imageFilter = (req, file, cb) => {
    ...
    if (!file.originalname.match(/\.(jpg|jpeg|png|gif)$/i)) {
        return cb(new Error('Lejohen vetëm skedarët e imazheve!'), false);
    }
    cb(null, true);
};
...
// cloudinary config
cloudinary.config({ 
  cloud_name: 'dmqecltxy', 
  api_key: 571843551756777, 
    ...
    Campground.find({name: regex}, function(err, allCampgrounds) {
      if (err) { console.log(err); }
        ...
        if (allCampgrounds.length < 1) {
          noMatch = "Nuk u gjet asnjë postim, ju lutem provoni përsëri.";
        }
    ...
    geocoder.geocode(req.body.campground.location, (err, data) => {
      ...
      Campground.findByIdAndUpdate(req.params.id, {$set: newData}, (err, updatedCampground) => {
        if (err) {
          ...
          req.flash("success","Posti u perditsua!");
...
router.delete("/:id", middleware.checkCampgroundOwenership, (req, res) => {
  Campground.findByIdAndRemove(req.params.id, err => {
    if (err) { res.redirect("/campgrounds"); }
      ...
      req.flash("success", "Posti u fshi!");
      res.redirect("/campgrounds"); }
  });
});
